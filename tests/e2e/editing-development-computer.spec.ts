...
test("completa el recorrido medible por CPU y mantiene la app operativa durante una demora", async ({ browser }) => {
  ...
  try {
    ...
    await application.page.getByLabel("Exposición").fill("0.2")
    await application.page.getByRole("button", { name: "Aplicar ajustes" }).click()
    await expect.poll(async () => (await application.state()).editingJobs[0].versions.length, { timeout: 12_000 }).toBe(2)
    await application.page.getByRole("button", { name: "Aprobar versión 2" }).click()
    await expect(application.page.getByRole("button", { name: "Iniciar sesión fotográfica" })).toBeEnabled()
    await application.page.getByRole("button", { name: "Iniciar sesión fotográfica" }).click()
    ...
    expect(state.editingJobs[0].versions[1].deliveryStatus).toBe("ready")
