...
Crear el primer módulo operativo de SmartStudio para abrir un evento, verificar que el entorno mínimo está disponible, recibir capturas RAW + JPEG, agruparlas en sesiones fotográficas y series, mostrar vistas previas y registrar una selección principal.
...
El resultado del módulo será una sesión fotográfica conservada en estado listo para edición, con hasta tres fotografías seleccionadas y exactamente una principal. La edición y las salidas posteriores pertenecen a otras especificaciones.

## User Stories

1. Como operador, quiero crear un evento con un nombre, para agrupar correctamente todas las sesiones fotográficas de una boda o actividad.
...
9. Como operador, quiero terminar la sesión fotográfica activa cuando el espacio pase a crítico, para no perder un trabajo en curso.
10. Como operador, quiero impedir sesiones fotográficas nuevas cuando el espacio sea crítico y no exista un SSD verificado, para proteger Windows y los archivos del evento.
...
21. Como operador, quiero iniciar una sesión fotográfica con un número automático y hora, para identificarla sin introducir datos obligatorios.
22. Como operador, quiero añadir una etiqueta opcional a la sesión fotográfica, para reconocer un grupo cuando resulte útil.
23. Como operador, quiero mantener solo una sesión fotográfica activa, para impedir que fotografías de grupos diferentes se mezclen.
24. Como operador, quiero cancelar una sesión fotográfica sin borrar sus capturas, para manejar abandonos o errores sin pérdida de datos.
25. Como operador, quiero restaurar una sesión fotográfica cancelada, para corregir una cancelación accidental.
26. Como operador, quiero iniciar manualmente una serie dentro de la sesión fotográfica, para decidir qué capturas pertenecen a la misma ronda.
...
28. Como operador, quiero crear varias series en una misma sesión fotográfica, para repetir poses sin perder las fotografías anteriores.
...
39. Como operador, quiero que la aplicación rechace un estado con más de tres seleccionadas o sin principal, para evitar sesiones fotográficas ambiguas.
40. Como operador, quiero que toda acción se guarde automáticamente, para no depender de un botón manual de guardado.
41. Como operador, quiero recuperar el evento, la sesión fotográfica, las series y la selección después de reiniciar la aplicación, para continuar desde el último estado confirmado.
...
48. Como responsable del producto, quiero verificar al menos 150 sesiones fotográficas y 1.500 capturas RAW + JPEG por evento, para tener margen suficiente para una boda típica.
49. Como operador, quiero que el módulo produzca una sesión fotográfica claramente lista para edición, para continuar posteriormente sin reinterpretar la selección.
...
- Solo puede existir un evento operativo activo y una sesión fotográfica activa dentro de ese evento.
- Un evento requiere nombre y fecha/hora automática; ubicación y notas son opcionales.
- Una sesión fotográfica recibe número consecutivo, hora y etiqueta opcional.
- Una sesión fotográfica contiene una o más series; el operador inicia y cierra cada serie manualmente.
...
- Una sesión fotográfica permite hasta tres selecciones y exige exactamente una principal antes de quedar lista para edición.
- Las capturas excluidas, sesiones fotográficas canceladas y series anteriores se conservan y pueden recuperarse.
...
- Con espacio crítico y sin SSD se permite terminar la sesión fotográfica activa, pero se bloquea el inicio de una nueva.
...
- El módulo debe soportar como objetivo 150 sesiones fotográficas y 1.500 pares RAW + JPEG por evento.
...
- El escenario principal recorrerá creación de evento, verificación previa, sesión fotográfica, recepción RAW + JPEG, varias series, advertencias de calidad, selección, principal, reinicio y recuperación.
...
- Se realizará una prueba de capacidad con el equivalente a 150 sesiones fotográficas y 1.500 pares de archivos, midiendo estabilidad, almacenamiento y tiempos de respuesta.
